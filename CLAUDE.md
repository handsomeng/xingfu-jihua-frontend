...
- **2026-05-28**：沉浸式 v3 设计稿全量重构 + v2 费曼版全 71 天内容上线。**视觉层**：合并 Claude Design 给的两轮 patch（`for-your-repo` + `for-your-repo 2`），覆盖 9 个 HTML（index/map/day/reading/report/onboarding/catalog/journal/reports）+ 9 个 JS（home/pathmap/app/report/onboarding/catalog/journal/reports）+ 新增 `assets/immersive.css`（1371 行，接管所有屏，styles.css 保留备用但不再被引用）。STAGES 配色从沉稳土色统一到明媚版（觉察 #F09866 桃霞 / 探索 #E5B14B 麦金 / 练习 #7BBE7E 嫩芽 / 内化 #7AA8D6 晴空），`curriculum.js` 和 `styles.css --stage-N` 变量同步更新。底部胶囊 nav（今天/目录/路径）替代顶部一排小图标，day.html 加返回上一页按钮（左 ← 右 ✕），`app.js` 加 `state.history` 栈 + `goPrev()` 函数。**内容层**：`readings/day1-71.md` 71 份阅读全量覆盖（cp 自 `~/Obsidian/9-幸福计划/v2-费曼版/`，文件名重映射 `Day01-阅读-XX.md` → `day1.md`）；`assets/days/day1-71.json` 71 份卡片流通过 Python parser（`/tmp/v2_md_to_json.py`）从 v2 markdown 批量生成，支持 7 种题型（content/single_choice/multiple_choice/true_false/text/scale/fill_blank），处理觉察题 vs 知识题分类、正确答案、容错答案、占位提示、反馈映射。`curriculum.js` AVAILABLE_DAYS 扩到 [0..71]。线上 71 天全部可玩。Day 0 入门保留原版不变。
- **2026-05-28 晚**：修复 5/28 parser 系统性数据 bug（致命，整套 71 天内容此前大面积坏但未暴露）。**根因**：5/28 那版 Python parser 是纯正则文本映射，没对照 app.js 渲染契约，且覆盖了原本人工翻译正确的 Day 0-26。**8 类致命 bug**：① 71 天全无 summary 页（markDayCompleted 永不触发，任何一天都标不了完成）② true_false 140 处缺 options + correctAnswer 是布尔（app.js 用 renderSingleChoice 渲染会白屏）③ scale 68 处缺 options（同样白屏）④ multiple_choice 211 处 correctAnswers 是 `["opt1, opt2"]` 单字符串而非数组（永远判不对卡死）⑤ reading 锚点页 68 处被降级成空 body content。**重写 parser**（`scripts/v2_md_to_json.py`）严格对照契约：true_false/scale 跟 single_choice 同构（抓选项块+逐项反馈，仅 type 不同）/ multiple_choice 的 correctAnswer 拆逗号成数组 + 「全选正确」「答对」→allCorrectFeedback + 「漏选/错选「optN」」→逐项 / 最后一页 content(next=无)→type:summary / 「附属阅读」锚点页→type:reading + source:readings/dayN.md / 非末页缺 next 自动补成下一页 id（修 Day1-3 p3 漏 next 的回归）。诊断脚本 `scripts/scan_day_bugs.js` 对照契约扫描，8 类致命 bug 全部清零，71 天链路模拟从 p1 走通到 summary。**遗留**：day31-71 部分觉察题反馈在 markdown 源里是占位「分别给针对性反馈」或措辞不一，导致约 2700 处选项 feedback 空（仅影响弹窗体验，不卡死不影响判对），属内容缺口，后续单独补。
- **2026-05-31**：处理 Codex 代码 review 的 3 个快修项（逐条实测核实属实）。**#5 Day0 本地报告 bug**：`api-proxy/server.js` 的 `/api/report` 守卫 `if (!day || !theme)` 把 Day0(day=0 falsy) 当缺参拒掉返回 400，本地 Day0 AI 报告生成失败（Vercel Edge 版 `api/report.js` 用 `day == null` 正确放行，故仅影响本地，线上无碍）。这是 5/24「Day0 falsy 坑」修了 home.js/catalog.js 但漏了本地 server.js 的同源遗漏。改成 `day == null || !theme`。**#2 scan 脚本 ESM**：`scripts/scan_day_bugs.js` 用 require 但根 package.json 是 `type:module`，`node scripts/scan_day_bugs.js` 直接报错。改成 import + `import.meta.url` 相对路径（顺便去掉硬编码绝对路径）。根 `package.json` 加 scripts：`dev`(起 api-proxy) / `validate:data` / `check`(跑 scan) / `build:data`(跑 v2 parser)。**#3 文档过期**：README 还写「MVP Day1-6 已完成 / Day14-34 待产出 / ?day=1..6」、BACKLOG「P2 拓展到 Day4-71」，实际 71 天全上线。README 进度表改成 Day0-71 全量已上线 + 标注内容源/转换脚本/校验命令 + 遗留 feedback 占位；BACKLOG P2 标 ✅ 已完成。**未处理（Codex 也提了，按节奏排期）**：#1 补 2700 处 feedback 占位（内容量大）、#4 兑换码后端化 + /api/report 鉴权限流（正式收费前）、#6 AI 报告加 DOMPurify（自看场景风险低）、#7 PWA、#8 设置页做真。
